...
//				jQuery(this).addClass(dialog_active_class);
		...
		dialog_.dialog({
			...
			width        : dialog_width,
			open         : function(event, ui){
				body_var.addClass('dialog_regular_open blur_content');
			},
			close        : function(event, ui){
				body_var.removeClass('dialog_regular_open blur_content');
			}
		});
...
jQuery.fn.DialogFixed = function(height){
	...
	if(height != null){
		element.css('top', Math.max((global_window_Height / 2) - (height / 2), 0));
		//console.log(global_window_Height);
		//console.log(height);
	}
	else{
		if(element.height() > global_window_Height){
			element.css('top', doc_var.scrollTop());
		} else{
			element.css('top', doc_var.scrollTop() + Math.max((global_window_Height / 2) - (element.outerHeight() / 2), 0));
		}

		//console.log((global_window_Height / 2) - (element.outerHeight() / 2) || 0);
	}
};